...
size_combobox = customtkinter.CTkComboBox(root, values=size_options_combobox)
size_combobox.grid(row=0, column= 0)
size_combobox.set("Random")
#Display label and update text with choice 
display_size = customtkinter.CTkLabel(root, text='')
display_size.grid(row=0, column=2)
#Create button to add a size
add_size_btn = customtkinter.CTkButton(root, text="Add Size", command=add_size)
